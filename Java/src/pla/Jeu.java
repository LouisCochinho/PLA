...
public class Jeu extends BasicGameState {
	...
	public void render(GameContainer gc, StateBasedGame game, Graphics g) throws SlickException {
		...
		if (!gc.isPaused()) {
			...
			ImageNoire.draw(0,0,gc.getWidth(),gc.getHeight());   // Taille de la fenetre modifi√©e par Antoine
	...
	// Met ÔøΩ jour les ÔøΩlÔøΩments de la scÔøΩne en fonction du delta temps
	// survenu.
	// C'est ici que la logique du jeu est enfermÔøΩe.
	@Override
	public void update(GameContainer gc, StateBasedGame game, int delta) throws SlickException {
		...
		for (Personnage p : personnages) {
			if (p.isDeplacementTermine()) {
				...
				System.out.println("Tableau aprËs : \n");
				p.getAutomate().afficher();
			}
